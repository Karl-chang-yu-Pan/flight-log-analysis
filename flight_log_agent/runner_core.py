...
async def analyze_flight_log(
    ...
    try:
        ...
        if cached_candidates and source_path_obj is None:
            ...
            # ------------------------------------------------------------
            # No pinned source snapshot and no usable cache hits:
            # deterministic model-free unresolved result (Stage-2
            # S2). Legacy iterative discovery is deleted; nothing
            # here may construct the legacy resolver or call a
            # provider.
            # ------------------------------------------------------------
            source_evidence = empty_source_discovery_evidence(source_search_context)
            candidate_set = MechanismCandidateSet(
